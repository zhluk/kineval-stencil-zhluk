    ...
    for (x in curRobot.joints) {
        ...
        if ( (typeof curRobot.joints[x].type !== 'undefined')
            ...
            if (curRobot.joints[x].type ==='revolute' || curRobot.joints[x].type ==='prismatic'){
                if (curRobot.joints[x].angle + curRobot.joints[x].control >= curRobot.joints[x].limit.upper){
                    curRobot.joints[x].angle = curRobot.joints[x].limit.upper;
                } else if (curRobot.joints[x].angle + curRobot.joints[x].control <= curRobot.joints[x].limit.lower){
...
//console.log(curRobot); 
    ...
    if (curRobot.links_geom_imported){
        ...
        curRobot.origin.rpy[1] += curRobot.control.rpy[0];
        curRobot.origin.rpy[2] += curRobot.control.rpy[1];
        curRobot.origin.rpy[0] += curRobot.control.rpy[2];
        

        // move camera with robot base
        camera_controls.object.position.z += curRobot.control.xyz[0];
        camera_controls.object.position.x += curRobot.control.xyz[1];
        camera_controls.object.position.y += curRobot.control.xyz[2];
    }
    else {
        curRobot.origin.xyz[0] += curRobot.control.xyz[0];
        curRobot.origin.xyz[1] += curRobot.control.xyz[1];

...
function administerCompetition(pendulum) {
	if (typeof competition === 'undefined') {
		competition = {state: "not_satisfied", satisfiedTime: null, setpointsReached: 0, startTime: Date.now(), satisfiedCount: 0, totalCount: 0, frameStartTime: 0}
	}
...
function checkSetpoint(pendulum) {
	...
	if (satisfiedTest(pendulum)) {
		if (competition.state === "not_satisfied") {
			...
			competition.frameStartTime = Date.now();
			return
		}
		else {
			if ((((Date.now() - competition.frameStartTime)) >= 1000) && (Date.now() - competition.satisfiedTime) <= 5000) {
				competition.satisfiedCount += 1;
				competition.frameStartTime = Date.now();
			}
